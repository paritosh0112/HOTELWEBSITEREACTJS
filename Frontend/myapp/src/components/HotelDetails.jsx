...
const HotelDetails = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const h = res.data.find((h) => h._id === id);
        setHotel(h);
      })
  ...
  return (
    <>
              ...
              <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1 text-sm font-bold text-yellow-400 mb-1">
                      <span>⭐</span>
                      {hotel.rating || "4.5"}
                      <span className="text-gray-500 font-normal">({hotel.reviews || 0} reviews)</span>
                    </div>
                ...
                <div className="border-t border-gray-100 pt-4 mb-6">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-lg text-gray-500 line-through">₹{hotel.originalPrice || "2500"}</span>
                    <span className="text-sm text-green-600 font-semibold">per night</span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1">Taxes included • No hidden charges</p>
                </div>
              </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {hotel.amenities.map((amenity, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-sm text-gray-700 p-2 hover:bg-gray-50 rounded-xl transition-colors">
                        <span className="w-1.5 h-1.5 bg-green-500 rounded-full" />
                        {amenity}
                      </div>
                    ))}
              ...
              <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-2 p-3 bg-purple-50 rounded-2xl border border-purple-100">
                    ...
                    <span className="font-semibold text-purple-800">Clean & Safe</span>
                  </div>
                </div>
      ...
      {/* FIXED Full Screen Booking Form */}
      {showBooking && hotel && (
